<script setup>
    ...
    const buttonCreate = (TaskName, TaskDescription, TaskCategory, TaskTag, TaskDate) => {
      store.addNewTask(TaskName, TaskDescription, TaskCategory, TaskTag, TaskDate)
      closeCartF()
    }
...
<template>
  ...
  <div class="fixed inset-0 flex items-center justify-center z-20">
    <div class="bg-white w-full max-w-3xl p-8 shadow-xl relative">
      ...
      <h3 class="text-2xl text-center mb-5">Создание задачи</h3>
      <div class="flex justify-between">
        <div class="flex flex-col w-96">
          <input class="tuk p-2 mb-5" style="border: 1px solid black;" type="text" placeholder="Название задачи" v-model="TaskName">
          <textarea class="p-2" style="border: 1px solid black;" placeholder="Описание задачи" v-model="TaskDescription"></textarea>
        </div>
      ...
      <a href="#" class="block text-white w-40 py-2 text-center m-auto" style="background-color: #E5662F;" @click="buttonCreate(TaskName, TaskDescription, TaskCategory, TaskTag, TaskDate)">СОЗДАТЬ</a>
    </div>
  </div>
</template>
